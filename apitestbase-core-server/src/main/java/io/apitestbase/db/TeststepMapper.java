...
public class TeststepMapper implements RowMapper<Teststep> {
    public Teststep map(ResultSet rs, StatementContext ctx) throws SQLException {
        ...
        if (fields.contains("api_request") && rs.getString("api_request") != null) {
            try {
                teststep.setApiRequest(
                        new ObjectMapper().readValue(rs.getString("api_request"), APIRequest.class));
            } catch (IOException e) {
        ...
        teststep.setEndpointProperty(
                fields.contains("endpoint_property") ? rs.getString("endpoint_property") : null);

        if (fields.contains("run_pattern") && rs.getString("run_pattern") != null) {
            try {
                teststep.setRunPattern(
                        new ObjectMapper().readValue(rs.getString("run_pattern"), TeststepRunPattern.class));
            } catch (IOException e) {
                throw new SQLException("Failed to deserialize run_pattern JSON.", e);
            }
        }
